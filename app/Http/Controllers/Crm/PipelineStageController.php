...
class PipelineStageController extends Controller
{
    ...
    {
        $stages = PipelineStage::withCount('offers')
            ->ordered()
            ->get();

        return Inertia::render('Admin/Crm/PipelineStages/Index', [
            'stages' => $stages,
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $validated['sort_order'] = PipelineStage::max('sort_order') + 1;
    ...
    {
        if ($stage->offers()->exists()) {
            return back()->with('error', 'Bu aşamada teklif bulunuyor, silinemez.');
        }
