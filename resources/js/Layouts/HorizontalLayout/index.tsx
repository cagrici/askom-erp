...
const HorizontalLayout = (props: any) => {
    ...
    // Filter out items where visible is explicitly false
    const filteredNavData = (navData || []).filter((item: any) => item.visible !== false);
    let menuItems: any = [];
    let splitMenuItems: Array<any> = [];
    let menuSplitContainer = 7;
    filteredNavData.forEach(function (value: any, key: number) {
        if (value['isHeader']) {
            menuSplitContainer++;
        }
    ...
    if (splitMenuItems.length > 0) {
        menuItems.push({ id: 'more', label: 'Diğer', icon: 'ri-briefcase-2-line', link: "/#", stateVariables: isMoreMenu, subItems: splitMenuItems, click: function (e: any) { e.preventDefault(); setIsMoreMenu(!isMoreMenu); }, });
    }
