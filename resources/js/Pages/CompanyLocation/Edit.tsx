...
const EditLocation: React.FC<EditLocationProps> = ({ location, parentLocations, locationTypes }) => {
  ...
  const { data, setData, put, processing, errors, reset } = useForm({
    ...
    type: location.type || "",
    location_type_id: location.location_type_id ? location.location_type_id.toString() : "",
    is_active: location.is_active,
    latitude: location.latitude,
    longitude: location.longitude,
    meta_data: location.meta_data || {},
    parent_id: location.parent_id ? location.parent_id.toString() : ""
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    put(route('company-locations.update', location.id));
  };
  ...
  return (
    <>
      <Head title={t("Edit Location") + " | Portal"} />
      <div className="page-content">
        <Container fluid>
          <BreadCrumb title={t("Edit Location")} pageTitle={t("Location Management")} />
                          ...
                          >
                            ...
                            {parentLocations.map((parentLocation) => (
                              <option
                                ...
                                disabled={parentLocation.id === location.id}
                              >
                                {parentLocation.name} {parentLocation.id === location.id ? `(${t("Current")})` : ""}
                              </option>
                            ))}
                          </Form.Select>
                  ...
                  <Card.Body>
                    <div className="d-grid gap-2">
                      <Button type="submit" variant="primary" disabled={processing}>
                        {processing ? t("Saving...") : t("Update Location")}
                      </Button>
...
EditLocation.layout = (page: any) => <Layout children={page} />;
