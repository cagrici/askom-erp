...
export default function Edit({ priceList, types, currencies, customerGroups }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('sales.price-lists.update', priceList.id));
    };

    const handleCustomerGroupChange = (groupValue: string, checked: boolean) => {
        if (checked) {
            setData('customer_groups', [...data.customer_groups, groupValue]);
        } else {
            setData('customer_groups', data.customer_groups.filter(value => value !== groupValue));
        }
    };

    return (
        <Layout>
            <Head title={`Fiyat Listesi Düzenle - ${priceList.name}`} />
                ...
                <div className="d-flex justify-content-between align-items-center mb-4">
                    <div>
                        ...
                        <p className="text-muted mb-0">
                            {priceList.name} - <code>{priceList.code}</code>
                        </p>
                    </div>
                                            ...
                                            {data.currency !== priceList.currency && (
                                                <Alert variant="warning" className="mt-2 mb-0">
                                                    <small>
                                                        ...
                                                        Para birimi değişikliği mevcut fiyatları etkileyebilir!
                                                    </small>
                                                </Alert>
                                            )}
                                        </Col>
                                    ...
                                    <Card.Body>
                                        ...
                                        <Row>
                                            {customerGroups.map((group) => (
                                                <Col md={6} key={group.value} className="mb-2">
                                                    <Form.Check
                                                        type="checkbox"
                                                        id={`group-${group.value}`}
                                                        label={group.label}
                                                        checked={data.customer_groups.includes(group.value)}
                                                        onChange={(e) => handleCustomerGroupChange(group.value, e.target.checked)}
                                                    />
                                                </Col>
                                            ))}
                                        </Row>
                        ...
                        <Col lg={4}>
                            <Card className="mb-4">
                                ...
                                <Card.Body>
                                    ...
                                    {data.is_default && !priceList.is_default && (
                                    ...
                                    {!data.is_active && priceList.is_default && (
                                        <Alert variant="warning" className="mb-0">
                                            <small>
                                                <i className="ri-alert-line me-1"></i>
                                                Varsayılan fiyat listesini pasif yapmak önerilmez!
                                            </small>
                                        </Alert>
                                    )}
                                </Card.Body>
                            ...
                            <Card className="mt-3">
                                <Card.Header>
                                    <h6 className="mb-0">Fiyat Listesi Bilgileri</h6>
                                </Card.Header>
                                <Card.Body>
                                    <div className="d-flex justify-content-between mb-2">
                                        <span className="text-muted">Oluşturulma:</span>
                                        <span>{new Date(priceList.created_at || '').toLocaleDateString('tr-TR')}</span>
                                    </div>
                                    <div className="d-flex justify-content-between mb-2">
                                        <span className="text-muted">Son Güncelleme:</span>
                                        <span>{new Date(priceList.updated_at || '').toLocaleDateString('tr-TR')}</span>
                                    </div>
                                    <div className="d-flex justify-content-between">
                                        <span className="text-muted">Ürün Sayısı:</span>
                                        <span className="fw-medium">{(priceList as any).prices_count || 0}</span>
                                    </div>
                                </Card.Body>
                            </Card>
                        </Col>
